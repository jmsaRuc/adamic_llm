...
This module provides functionality to run LangGraph models
with an OpenAI-compatible interface. It handles conversion
between OpenAI's message format and LangChain's message
format, and provides both streaming and non-streaming
interfaces for running LangGraph workflows.
...
Examples:
    ...
    >>> async for chunk, metrics in run_langgraph_stream(
    ...     "my-model", messages, registry,
    ... ):
    ...     print(chunk)

The module contains the following functions:
- `convert_to_lc_messages(messages)` - Converts OpenAI
  messages to LangChain messages.
- `register_graphs(graphs)` - Validates and returns the
  provided graph dictionary.
- `run_langgraph(model, messages, graph_registry)` - Runs
  a LangGraph model with the given messages.
...
from adamic_llm.utils.message import convert_to_lc_messages
