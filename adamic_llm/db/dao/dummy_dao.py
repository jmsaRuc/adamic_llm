...
from adamic_llm.db.models.dummy_model import DummyModel
...
class DummyDAO:
    ...
    async def create_dummy_model(self, name: str) -> None:
        ...
        """
        async with (
            self.db_pool.connection() as connection,
            connection.cursor(binary=True) as cur,
        ):
            await cur.execute(
    ...
    async def get_all_dummies(self, limit: int, offset: int) -> list[DummyModel]:
        ...
        async with (
            self.db_pool.connection() as connection,
            connection.cursor(binary=True, row_factory=class_row(DummyModel)) as cur,
        ):
