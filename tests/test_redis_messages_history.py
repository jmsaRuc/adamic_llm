...
async def test_adamic_history_google(fake_redis_pool: ConnectionPool) -> None:
    """Tests that if there are multiple human messages with a system message.
    ...
    state2 = SummaryState(messages=messages2)

    request_message_history2 = await get_request_message_history(state2)
    ...
    adamic_input_history, adamic_output_history = await get_adamic_history(
        request_message_history2, history_key2, fake_redis_pool, mode="adamic", translation_method="google"
    )
...
async def test_adamic_history_self_translate(fake_redis_pool: ConnectionPool) -> None:
    """Tests that if there are multiple human messages with a system message.
    
    The request message history is correct, and the Adamic history is correctly.
    retrieved from Redis and reconstructed into the correct format for the Adamic input
    and self-translate nodes."""
    ...
    test_adamic_input_ai_content = "Hovedstaden i dit land er København."
    test_adamic_ai_content = AIMessage(content=test_adamic_input_ai_content)
    key_lang = await create_key_lang(history_key, "da")
    test_adamic_input_ai_with_name = await insert_name_into_ai_message(
        test_adamic_ai_content, key_lang
    )
    assert test_adamic_input_ai_with_name.name == key_lang
    
    # Second run, simulating the next turn in the conversation
    messages2: list[AnyMessage]
    messages2 = [
        HumanMessage(content="hvad er hovedstaden i midt land?"),
        test_adamic_input_ai_with_name,
        HumanMessage(content="hvor mange indbyggere er der?"),
    ]
    state2 = SummaryState(messages=messages2)
    ...
    assert request_message_history2[0].content == "hvad er hovedstaden i midt land?"
    assert isinstance(request_message_history2[1], AIMessage)
    assert request_message_history2[1].content == test_adamic_input_ai_content
    assert request_message_history2[1].name == key_lang
    ...
    assert history_key == history_key2
    adamic_self_translate_history, adamic_input_history = await get_adamic_history(
        request_message_history2, history_key2, fake_redis_pool, mode="adamic", translation_method="self-translate"
    )
    
    lang_name = await get_lang_name(lang_code)
    assert lang_name == "Danish"
    
    test_adamic_self_translate_human_content = adamic_self_translate_user_prompt.format(
        question_none_en=messages2[0].content,
        detected_language=lang_name,
    )
    
    test_adamic_input_human_content = adamic_input_with_self_translate_user_prompt.format(
        question_en=test_translated_human_content,
        question_none_en=messages2[0].content,
        detected_language=lang_name,
    )
    
    assert adamic_self_translate_history is not None
    assert adamic_input_history is not None
    assert len(adamic_self_translate_history) == 2
    assert len(adamic_input_history) == 2
    assert adamic_self_translate_history[0].content == test_adamic_self_translate_human_content
    assert adamic_self_translate_history[0].type == "human"
    assert adamic_self_translate_history[1].content == test_translated_human_content
    assert adamic_self_translate_history[1].type == "ai"
    assert adamic_input_history[0].content == test_adamic_input_human_content
    assert adamic_input_history[0].type == "human"
    assert adamic_input_history[1].content == test_adamic_input_ai_content
    assert adamic_input_history[1].type == "ai"