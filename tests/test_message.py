...
from adamic_llm.web.api.chat.schema import ChatCompletionRequestMessage, Role
...
def test_assistant_message_conversion() -> None:
    ...
    messages = [
        ChatCompletionRequestMessage(
            role=Role.ASSISTANT, content="I'm doing well, thank you!"
        )
    ]
    result = convert_to_lc_messages(messages)
...
def test_mixed_message_conversion() -> None:
...
def test_none_content_handling() -> None:
...
def test_unsupported_role_types() -> None:
