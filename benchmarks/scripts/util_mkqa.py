...
def map_em_value(prediction, gold_answers, lang):
    em_value = compute_max_score_over_answers(
        calculate_em, prediction, gold_answers, lang
    )
    return float(em_value)


def map_f1_value(prediction, gold_answers, lang):
    f1_value = compute_max_score_over_answers(
        calculate_f1, prediction, gold_answers, lang
    )
...
def get_text_metrics(
    ...
    if serial:
        f1_scores = [
            map_f1_value(predictions[i], gold_answers[i], lang)
            for i in range(len(predictions))
        ]
        em_scores = [
            map_em_value(predictions[i], gold_answers[i], lang)
            for i in range(len(predictions))
        ]
    else:
        with Pool(workers) as p:
            f1_scores = p.starmap(
                map_f1_value,
                [
                    (predictions[i], gold_answers[i], lang)
                    for i in range(len(predictions))
                ],
                chunksize=64,
            )
...
def summarize_default_metrics(
    ...
    assert (
        set(em_scores.keys()) == set(f1_scores.keys()) == set(qid_is_answerable.keys())
    )
    ...
    unans_em_scores = {
        qid: em_scores[qid] for qid in em_scores if not qid_is_answerable[qid]
    }
    ...
    summary = OrderedDict(
        [
            ...
            (
                ...
                round(100.0 * np.mean(list(ans_em_scores.values())), 2),
            ),
            ("answerable_f1", round(100.0 * np.mean(list(ans_f1_scores.values())), 2)),
            (
                "unanswerable_exact_match",
                round(100.0 * np.mean(list(unans_em_scores.values())), 2),
            ),
        ]
    )
...
def plot_f1(
    ...
    for i, qid in enumerate(qid_list):
        ...
        if qid in answerable_f1_by_id:
            ...
            raise ValueError(
                f"{qid} is not in either answerable or unanswerable predictions"
            )
...
def calculate_f1(prediction, gold_answer, language):
    gold_toks = (
        normalize_answer_by_language(gold_answer, language).split()
        if gold_answer
        else []
    )
    pred_toks = (
        normalize_answer_by_language(prediction, language).split() if prediction else []
    )
...
def apply_no_answer_threshold(
    scores, no_answer_probs, qid_has_answer, no_answer_thresh
):
    new_scores = {}
...
class Rouge_Scorer:
    def __init__(
        self, model_name_or_path="gpt-3.5-turbo", metrics=["rouge1", "rougeL"]
    ):
        self.model_name_or_path = model_name_or_path
